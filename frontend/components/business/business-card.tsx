...
export function BusinessCard({ business, saved, onToggleSaved }: { business: Business; saved: boolean; onToggleSaved: () => void }) {
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl bg-surface shadow-lift transition-transform duration-200 hover:-translate-y-1">
      ...
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-line pt-4">
          {business.growthLabel ? <span className="text-xs font-semibold text-accent">{business.growthLabel}</span> : null}
          {business.seeking?.map((need) => <Badge key={need} tone="mint">Mencari {need}</Badge>)}
        </div>
